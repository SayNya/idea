...
class ClosePollHandler:
    ...
    status_to_key_mapping = {
        IdeaStatusCodeEnum.ACCEPTED.value: CouncilResultAttributeKeysEnum.ACCEPTED_IDEAS.value,
        IdeaStatusCodeEnum.UPDATING.value: CouncilResultAttributeKeysEnum.TO_UPDATING_IDEAS.value,
        IdeaStatusCodeEnum.DECLINED.value: CouncilResultAttributeKeysEnum.DECLINED_IDEAS.value,
    }
    ...
    async def handle(
        ...
        if poll.status != PollStatusesEnum.ENDED.value:
        ...
        ended_polls = await self.poll_repository.find_by_council_id_and_statuses(
            poll.council.id, [PollStatusesEnum.ENDED.value]
        )
        # close poll
        await self.poll_repository.update(
            ended_polls[0].id, {"status": PollStatusesEnum.CLOSED.value}
        )
        ...
        blocked_polls = await self.poll_repository.find_by_council_id_and_statuses(
            poll.council.id, [PollStatusesEnum.BLOCKED.value]
        )
        if blocked_polls:
            # open next poll
            await self.poll_repository.update(
                blocked_polls[0].id, {"status": PollStatusesEnum.OPENED.value}
            )
            return {}
        else:
            # or close council
